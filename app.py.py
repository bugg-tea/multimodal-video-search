...
drive.mount('provide drive link to your video')
...
def process_text_query(query_text):
    """
    Text → embedding → multimodal search
    """
...
def process_image_query(image_path):
    """
    Image → BLIP caption → text embedding → multimodal search
    """
    from PIL import Image
...
def process_audio_query(audio_path):
    """
    Audio → Whisper transcript → text embedding → multimodal search
    """
    try:
...
def search_video(selected_inputs, text_query, image_file, audio_file):
    ...
    if not (text_input or image_input or audio_input):
        return "❌ Please provide at least one input based on your selection."
